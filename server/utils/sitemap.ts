// apps/grumpy-old-blogger/server/utils/sitemap.ts
import type { SitemapUrl } from '#sitemap/types'

export async function generateSitemapUrls(event: any): Promise<SitemapUrl[]> {
  ...
  const [
    ...
    sanity.fetch<{ slug: string; _updatedAt: string }[]>(
      qryAllProductCollectionSlugs,
    ),
    sanity.fetch<{ slug: string; _updatedAt: string }[]>(
      qryAllArticleCategorySlugs,
    ),
